...
[one arm and a fixture](../one-arm-training/overview.md). The rest of the folder is
...
by running [the one-arm methods](../one-arm-training/overview.md) twice, and the
...
this folder and in
[the one-arm documents](../one-arm-training/overview.md#1-what-an-arm-is-actually-asked-to-do).
...
6. **Otherwise: one arm and a fixture.** Read
   [the one-arm documents](../one-arm-training/overview.md) and spend the saved
...
Next: [the two-arm overview](overview.md) for what actually changes, or
[the one-arm documents](../one-arm-training/overview.md) if the checklist sent you
back there.
