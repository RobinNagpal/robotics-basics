...
trigonometry in it. No ``cos``, no ``sin``, no ``q1 + q2``. It does not import
``arm_math``. It does not know how long the links are, how many joints the arm
has, or that the arm is flat.
...
class GripperWatcher(Node):
    ...
    def _on_timer(self) -> None:
        ...
        self.get_logger().info(
            f'gripper at ({x:+.3f}, {y:+.3f}) facing {math.degrees(yaw_of(found)):+7.1f}°   '
