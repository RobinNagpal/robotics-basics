...
- [ROS camera](03_ros-camera.md): one program publishes pictures, and another
  receives them.
- [ROS arm](04_ros-arm.md): a program moves a two-joint arm by publishing where its
  joints should be.
- [ROS camera and arm](05_ros-camera-arm.md): a program watches the camera, and
...
| `PointCloud2` | a cloud of 3D points, usually from a depth camera | the [camera area](../05_camera/04_one-box-code.md) |
...
`reading.header.stamp.sec`. The [camera doc](03_ros-camera.md) explains the `Image`
and `CameraInfo` classes field by field, and the [arm doc](04_ros-arm.md) does the
...
topic, and publishes where every link is on TF. The [arm doc](04_ros-arm.md) shows
...
| --- | --- | --- |
| [topics](02_ros-basics.md#3-topics-publishing-and-subscribing), [parameters](02_ros-basics.md#4-parameters-a-nodes-settings), [launching](02_ros-basics.md#8-launching-starting-everything-together) | a publisher, a subscriber and a settings node, started together | `make ros.basics` |
...
| --- | --- | --- | --- |
| [ROS camera](03_ros-camera.md) | one node publishes pictures, another receives them and finds a ball | `make ros.camera` | nodes, publishers, subscribers, messages, timers |
| [ROS arm](04_ros-arm.md) | a node moves a two-joint arm and its gripper by publishing joint positions, and a distance sensor in the gripper measures the table | `make ros.arm` | URDF, joint states, robot_state_publisher, TF, a sensor message |
| [ROS camera and arm](05_ros-camera-arm.md) | a node watches the camera, and points the arm at the ball | `make ros.camera_arm` | one node that subscribes and publishes, and packages that reuse each other |
...
Next: [ROS basics](02_ros-basics.md), one small program for each of these ideas.
Then [ROS camera](03_ros-camera.md), the first worked example.
