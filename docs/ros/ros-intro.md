...
3. [Packages and the workspace](#3-packages-and-the-workspace)
4. [Starting nodes: ros2 run and launch files](#4-starting-nodes-ros2-run-and-launch-files)
5. [Looking inside a running robot](#5-looking-inside-a-running-robot)
6. [Frames, TF and URDF](#6-frames-tf-and-urdf)
7. [RViz](#7-rviz)
8. [The three examples](#8-the-three-examples)
...
named after the package that defines them. A picture is a `sensor_msgs/Image`,
and the `ros2 interface show` command lists what it holds:
...
## 4. Starting nodes: ros2 run and launch files
...
## 5. Looking inside a running robot
...
| [ROS arm](ros-arm.md) | a node moves a two-joint arm by publishing its joint angles | `make ros.arm` | URDF, joint states, robot_state_publisher, TF |
