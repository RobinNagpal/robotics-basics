...
SNAPSHOT: Path = Path(__file__).parent / 'data' / 'one_box'
...
def capture() -> dict[str, Any]:
    ...
    The values are messages of different classes, an Image, a CameraInfo and a
    TFMessage, so their type is written as Any: "any type at all".
    """
...
@pytest.fixture(scope='module')
def camera(capture: dict[str, Any]) -> PinholeCameraModel:
    """Read the lens numbers from the camera info."""
    model: PinholeCameraModel = PinholeCameraModel()
    model.from_camera_info(capture['/camera/camera_info'])
    return model


@pytest.fixture(scope='module')
def camera_to_world(capture: dict[str, Any]) -> NDArray[np.float64]:
    ...
    tf: TransformStamped = buffer.lookup_transform('world', 'camera_optical_frame', Time())
...
@pytest.fixture(scope='module')
def depth(capture: dict[str, Any]) -> NDArray[np.float32]:
...
def test_the_lens_is_the_one_the_doc_uses(camera: PinholeCameraModel) -> None:
...
def test_the_camera_hangs_40_cm_above_the_table_looking_down(
        camera_to_world: NDArray[np.float64]) -> None:
...
def test_the_table_reads_40_cm_and_the_box_top_34_cm(depth: NDArray[np.float32]) -> None:
...
def test_the_doc_pixel_becomes_the_doc_point(
        depth: NDArray[np.float32], camera: PinholeCameraModel) -> None:
    """One-box intro, section 1.1: pixel (212.5, 86.5) is the point (+0.0644, -0.0411, 0.340)."""
    points: NDArray[np.float64] = depth_to_points(
        depth, camera.fx(), camera.fy(), camera.cx(), camera.cy())
    assert points[86, 212] == pytest.approx([0.0644, -0.0411, 0.340], abs=5e-5)


def test_the_box_is_measured(
        depth: NDArray[np.float32], camera: PinholeCameraModel,
        camera_to_world: NDArray[np.float64]) -> None:
    """One-box intro, section 1.4: the middle is within 1 mm of (0.065, 0.040), 6 cm tall."""
    points: NDArray[np.float64] = depth_to_points(
        depth, camera.fx(), camera.fy(), camera.cx(), camera.cy())
    box: BoxMeasurement | None = measure_box(to_world(points, camera_to_world))
...
def test_a_bare_table_has_no_box() -> None:
    # 5 rows of 5 points each, and 3 numbers per point: x, y and z. Every point
    # lies on the table, at height z = 0. Their x and y are left as NaN ("not a
    # number"), because a bare table has no box, wherever the points are.
    table: NDArray[np.float64] = np.full((5, 5, 3), np.nan)
    # [:, :, 2] picks, in every row (the first :) and every column (the second
    # :), the number at position 2 of the three: z.
    table[:, :, 2] = 0.0
