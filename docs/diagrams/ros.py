...
IMAGES = REPO_ROOT / 'docs' / 'images' / 'ros'
URDF = REPO_ROOT / 'src' / 'ros' / 'ros_arm' / 'urdf' / 'arm.urdf'
...
def picture():
    ...
    image = draw_picture(u, v)
    found = find_ball(image)
...
def image_message():
    ...
    tiny = [[(220, 40, 40), (200, 200, 200), (200, 200, 200)],
...
def _urdf_sizes():
    ...
    z = [float(joints[name].find('origin').get('xyz').split()[2]) for name in ('pan', 'tilt')]
    length = float(joints['tip_joint'].find('origin').get('xyz').split()[0])
...
def arm():
    ...
    reach = length * math.cos(tilt)
    tip = (reach * math.cos(pan), reach * math.sin(pan))
...
def pixel_to_angle():
    ...
    u, v = find_ball(draw_picture(*ball_position(2.0)))
