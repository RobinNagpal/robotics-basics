...
def test_the_picture_is_320_by_240_with_three_colours_per_pixel() -> None:
    assert draw_picture(160, 120).shape == (HEIGHT, WIDTH, 3) == (240, 320, 3)


def test_the_subscriber_finds_the_ball_where_it_was_drawn() -> None:
    u, v = find_ball(draw_picture(100.5, 80.5))
    assert (u, v) == pytest.approx((100.5, 80.5), abs=0.1)


def test_a_picture_without_the_ball_has_no_ball() -> None:
    assert find_ball(draw_picture(-100, -100)) is None


def test_the_ball_stays_inside_the_picture() -> None:
    for tenth in range(600):
        u, v = ball_position(tenth / 10)
        assert 15 <= u <= WIDTH - 15 and 15 <= v <= HEIGHT - 15


def test_k_puts_the_lens_numbers_where_programs_look_for_them() -> None:
    k = camera_info().k
    assert (k[0], k[4], k[2], k[5]) == (277.1, 277.1, 160.0, 120.0)     # fx, fy, cx, cy


def test_the_standard_camera_library_reads_the_same_lens() -> None:
