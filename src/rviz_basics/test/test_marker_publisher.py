...
def test_orbit_starts_on_positive_x_axis() -> None:
    x: float
    y: float
...
def test_orbit_reaches_positive_y_axis_at_quarter_period() -> None:
    x: float
    y: float
...
def test_orbit_is_periodic() -> None:
    start: tuple[float, float, float] = circular_orbit(0.0, RADIUS, PERIOD)
    full_lap: tuple[float, float, float] = circular_orbit(PERIOD, RADIUS, PERIOD)
...
def test_orbit_stays_on_the_circle(elapsed: float) -> None:
    x: float
    y: float
...
def test_yaw_is_tangent_to_the_orbit(elapsed: float) -> None:
    """Heading must be perpendicular to the radius — i.e. along the path."""
    x: float
    y: float
    yaw: float
    x, y, yaw = circular_orbit(elapsed, RADIUS, PERIOD)
    radial: tuple[float, float] = (x / RADIUS, y / RADIUS)
    heading: tuple[float, float] = (math.cos(yaw), math.sin(yaw))
...
def test_zero_yaw_is_identity_quaternion() -> None:
