...
QUARTER_TURN: float = math.pi / 2


def test_rotating_x_axis_by_a_quarter_turn_gives_the_y_axis() -> None:
    x: float
    y: float
...
def test_rotating_keeps_the_distance_from_the_origin() -> None:
    x: float
    y: float
...
def test_applying_a_pure_shift_just_adds() -> None:
    shift: Transform2D = Transform2D.translation(1.0, 2.0)
    assert shift.apply(0.5, 0.5) == pytest.approx((1.5, 2.5))


def test_joining_adds_the_angles() -> None:
    joined: Transform2D = Transform2D.rotation(0.4).then(Transform2D.rotation(0.25))
    assert joined.theta == pytest.approx(0.65)


def test_joining_rotates_the_child_offset() -> None:
    """A quarter turn, then a step along X, should end up along +Y."""
    joined: Transform2D = Transform2D.rotation(QUARTER_TURN).then(Transform2D.translation(1.0, 0.0))
    assert (joined.x, joined.y) == pytest.approx((0.0, 1.0), abs=1e-12)


@pytest.mark.parametrize('q1,q2', [(0.0, 0.0), (0.5, -0.3), (1.2, 0.9), (-0.7, 2.0)])
def test_inverse_undoes_the_transform(q1: float, q2: float) -> None:
    forward: Transform2D = gripper_in_base(q1, q2)
    back: Transform2D = forward.inverse().inverse()
...
def test_a_point_survives_a_round_trip_between_frames(q1: float, q2: float) -> None:
    """Into the gripper frame and back out should land on the same point."""
    base_to_gripper: Transform2D = gripper_in_base(q1, q2)
    there: tuple[float, float] = base_to_gripper.inverse().apply(0.31, -0.12)
    back: tuple[float, float] = base_to_gripper.apply(*there)
    assert back == pytest.approx((0.31, -0.12))


@pytest.mark.parametrize('q1_deg,q2_deg', [(0, 0), (45, 0), (0, 90), (45, 45), (90, -45)])
def test_joining_links_agrees_with_the_hand_derived_formula(q1_deg: int, q2_deg: int) -> None:
    ...
    joined: Transform2D = gripper_in_base(q1, q2)
    assert (joined.x, joined.y) == pytest.approx(gripper_position(q1, q2))


def test_straight_arm_reaches_the_sum_of_the_link_lengths() -> None:
    straight: Transform2D = gripper_in_base(0.0, 0.0)
