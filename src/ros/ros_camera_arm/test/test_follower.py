...
LENS = (277.1, 277.1, 160.0, 120.0)        # fx, fy, cx, cy


def test_the_middle_of_the_picture_is_straight_ahead():
    assert pixel_to_angles(160.0, 120.0, *LENS) == pytest.approx((0.0, 0.0))


def test_right_of_the_middle_turns_right_and_above_it_tilts_up():
    pan, tilt = pixel_to_angles(200.0, 100.0, *LENS)
    assert pan < 0 and tilt > 0


def test_one_focal_length_to_the_side_is_45_degrees():
